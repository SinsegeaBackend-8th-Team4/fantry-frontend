...
import { searchNotices } from '@/api/notice'; // Import actual API
...
async function fetchNotices({ page, size, sort }) {
  try {
    const params = {
      page,
      size,
      ...
      status: 'ACTIVE', // Assuming user notices are always active
    };
...
const goToDetail = (noticeId) => {
  router.push({ name: 'UserNoticeDetail', params: { noticeId } });
};
    ...
    <div class="card shadow-sm border-0 rounded-3 overflow-hidden mb-4">
      <div class="card-body p-4">
        ...
        <ServerDataTable
          ...
          :page-size="10"
          @row-click="({ row }) => goToDetail(row.noticeId)"
        >
          <template #empty>공지사항이 없습니다.</template>
        </ServerDataTable>
      </div>
    </div>
  </div>
